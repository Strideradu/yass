...
long int SortCriterionPercentIdentityQuery(MA *ma) {
    return (long int)
           ...
           (ma->transindels[0] + ma->transindels[1] +
            ma->transindels[3])  /* transitions+transversion+indels on query */
           *
           1000
           /
           (
...
long int SortBlocksCriterionQueryTextNumber(MA *ma) {
    return (long int) (ma->j_chunk) + (ma->i_chunk) * gp_nbchunks_text;
}
...
void MergeSort_forward_reverse(Feature *f1, Feature *f2, MA **p_first_MA, MA **p_last_MA) {
    ...
    if (f1->first_MA && f2->first_MA) {
        ...
        if (gp_sortcriterion >= 2 * NBPOTENTIALCRITERIA) {
            ...
            while (_ma_1 && _ma_2) {
                if ((_ma_1->i_chunk < _ma_2->i_chunk) ||
                    ((_ma_1->i_chunk == _ma_2->i_chunk) &&
                     (gp_sortcriterion_func(_ma_1) > gp_sortcriterion_func(_ma_2)))) {
                    last_MA->next = _ma_1;
                    last_MA = _ma_1;
                    _ma_1 = _ma_1->next;
                } else {
...
long int DisplayTuple(tuple *tuple) {
    ...
    printf("\t tuple : [(%9ld,%9ld),(%9ld,%9ld)] \t diag : %9ld \t size : %9ld \n", TBL_POS(tuple), TEL_POS(tuple),
           TBR_POS(tuple), TER_POS(tuple), tuple->diagonal, TSIZE(tuple));
    return 0;
}
...
long int DisplayMA(MA *ma) {
    ...
    while (tuple) {
        ...
        if (tuple->next) {
            printf("\t");
            DisplayGap(tuple, tuple->next);
        }
        tuple = tuple->next;
    }
    printf("\t = {MA : (%ld,%ld), (%ld,%ld) chunk:%ld, score:%ld, reverse:%d}\n", ma->left_pos_begin, ma->left_pos_end,
           ma->right_pos_begin, ma->right_pos_end, ma->i_chunk, ma->blastscore, ma->reverse);
    printf("\n");
    return 0;
}
