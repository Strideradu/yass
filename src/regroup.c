          ...
          goto cannotbegrouped;
            ...
            if (t_current_MA_p) {
                ...
                while (t_current_MA_p) {
                    while ((t_target_MA_c) && !(TL_POS(t_target_MA_c) >= TEL_POS(t_current_MA_p) &&
                                                TR_POS(t_target_MA_c) >= TER_POS(t_current_MA_p))) {
                ...
                if (t_current_best && t_target_best) {
                    while (TEL_POS(t_current_best) > TBL_POS(t_target_best) ||
                           TER_POS(t_current_best) > TBR_POS(t_target_best)) {
                ...
                t_target_MA_c = t_target_MA;
                while ((t_target_MA_c) && (TL_POS(t_target_MA_c) < TEL_POS(t_current_MA) ||
                                           TR_POS(t_target_MA_c) < TER_POS(t_current_MA))) {
                    t_target_MA_c = t_target_MA_c->next;
                }
...
#endif
                    while (TEL_POS(t_current_MA) > TBL_POS(t_target_MA_c) ||
                           TER_POS(t_current_MA) > TBR_POS(t_target_MA_c)) {
            ...
            if (l->ma == liste->ma) {
                hit = 0;
                    ...
                    ((ABS(liste->ma->right_pos_begin - l->ma->right_pos_end) + 1) >
                     MAX_WINDOW_REGROUP / (1 + 2 * gp_delta_stat))
                    ||
                    ...
                    (
                            ...
                            *
                            (
                                    (
                                            ABS(TGAP_R(l_ma_tuple_end, liste_ma_tuple_begin) -
                                                TGAP_L(l_ma_tuple_end, liste_ma_tuple_begin))
                                    )
                                    + 2 * gp_delta_stat + 1
                            )
    ...
    if (gp_win_mul > 1.0) {
        ...
        while (ma) {
            ...
            while (feature->windows <= gp_win_max) {
                ...
                feature->windows = (long int) (feature->windows * gp_win_mul) +
                                   1; /* FIXME: why + 1 here : check if winmin = 1 and gp_mul = 1 ... */
            }
            ma = chunkma;
        }
    }
