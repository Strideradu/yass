...
void Display_Alignements(MA *first_MA) {
    ...
    if (gp_display == 2) {
        ...
        fprintf(OUTSTREAM,
                "# s. id,\ts. start,\ts. end,\tstrand,\tq. start,\tq. end,\tbit score\te-value\t%%identity\n");
        ...
        switch (gp_display) {
            ...
            case 2:
                ...
                if (!(ma->reverse))
                    ...
                    fprintf(OUTSTREAM, "%ld\t%ld\t%ld\t%ld\t",
                            ...
                            BCOUNT + gp_chunksize_query[ma->j_chunk] - ma->left_pos_begin - 1,
                            BCOUNT + ma->right_pos_end -
                            1,                  /* inverted pair : blast database reversed */
                            BCOUNT + ma->right_pos_begin);
            ...
            case 5:
                ...
                fprintf(OUTSTREAM, "%ld\t%ld\t",
                        lowercaseCount((ma->reverse ? gp_query_rev : gp_query) + gp_chunkstrt_query[ma->j_chunk],
                                       ma->left_pos_begin, ma->left_pos_end - ma->left_pos_begin + 1),
                        nCount((ma->reverse ? gp_query_rev : gp_query) + gp_chunkstrt_query[ma->j_chunk],
                               ma->left_pos_begin, ma->left_pos_end - ma->left_pos_begin + 1));
...
void Display_Stats() {
...
#ifdef STATS
    {
        ...
        for (k = 0; k < ((gp_selection_fasta == 0) ? MAX_QUERY_CHUNK_THREADS : 1); k++) {
            ...
            /* regroup */
            fprintf(stderr, "  -\n");
            fprintf(stderr, "  # grouping tests   =   f:%9ld   r:%9ld   total:%9ld \n",
            ...
            fprintf(stderr, "  # align. linked    =   f:%9ld   r:%9ld   total:%9ld \n",
            ...
            fprintf(stderr, "  # align. postpros  =   f:%9ld   r:%9ld   total:%9ld \n",
            ...
            /* cpu-times */
            fprintf(stderr, "  ----\n");
            fprintf(stderr, "  pre-proc. cpu-time =   f:%8.2fs   r:%8.2fs   total:%8.2fs [%ld%%]\n",
            ...
            fprintf(stderr, "  %s cpu-time =   f:%8.2fs   r:%8.2fs   total:%8.2fs [%ld%%]\n",
            ...
            fprintf(stderr,
#ifdef THREAD_ASSEMBLE_ALIGN
                    "  align.(-) cpu-time =   f:%8.2fs   r:%8.2fs   total:%8.2fs [%ld%%]\n",
#else
                    "  align.    cpu-time =   f:%8.2fs   r:%8.2fs   total:%8.2fs [%ld%%]\n",
#endif
            ...
            fprintf(stderr, "  post-pro. cpu-time =   f:%8.2fs   r:%8.2fs   total:%8.2fs [%ld%%]\n",
    ...
    if (gp_nbfiles <= 1) {
        ...
        if (gp_selection_fasta == 0) {
            fprintf(stderr, "\"%s\", %ld chunk(s), (overall size : %ld bp, GC: %2.2f%%)\n", gp_files[0],
                    gp_nbchunks_query, gp_querysize, 100 * (gp_freq_letters[0][1/*C*/] + gp_freq_letters[0][2/*G*/]));
        } else {
            fprintf(stderr, "\"%s\", %ld chunk(s), selected chunk: %ld (size : %ld bp, GC: %2.2f%%)\n", gp_files[0],
                    gp_nbchunks_query, gp_selection_fasta, querysize_chunk,
                    100 * (gp_freq_letters[0][1/*C*/] + gp_freq_letters[0][2/*G*/]));
        }
    } else {
        ...
        fprintf(stderr, "\"%s\"  %ld chunk(s), (overall size : %ld bp,  GC: %2.2f%%)\n\n", gp_files[1],
                gp_nbchunks_text, gp_textsize, 100 * (gp_freq_letters[1][1/*C*/] + gp_freq_letters[1][2/*G*/]));
    }
...
void Display_Progress(long int pos, long int maxpos, Feature *feature) {
    ...
    if (gp_selection_fasta) {
        /* one query chunk */
        if (gp_reverse == 2) {
            while ((double) feature->last_point * maxpos < (double) pos * (NBL / 2) &&
                   (feature->last_point < (NBL / 2))) {
                fprintf(stderr, ".");
                fflush(NULL);
                (feature->last_point)++;
            }
        ...
        while ((double) feature->last_point * maxpos < (double) pos * (gp_dots[feature->j_chunk]) &&
               feature->last_point < gp_dots[feature->j_chunk]) {
            fprintf(stderr, ".");
            fflush(NULL);
            (feature->last_point)++;
        }
