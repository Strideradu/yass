...
static void parsepattern(int argc, char **argv, long int *i) {
    ...
    if (!(strchr(argv[*i], '#') || strchr(argv[*i], '@')))
        error(" pattern \"", argv[*i], "\"  doesn't have any \"#\" or \"@\" symbol");
 ...
 */
static void parsedlint(int argc, char **argv, long int *i, long int *var1, long int min1, long int max1, long int *var2,
                       long int min2, long int max2) {
...
static void
parseddouble(int argc, char **argv, long int *i, double *var1, double min1, double max1, double *var2, double min2,
             double max2) {
                ...
                for (j = 0; j < 4; j++) {
                    long int score = (long int) (
                            ...
                            gp_substitution_matrix[i + di][j + dj] * ABS(i == j
                                                                         ?
                                                                         log(0.01 + gp_freq_background[i][j]) /
                                                                         log(0.0625)
                                                                         :
                                                                         log(0.0625) /
                                                                         log(0.01 + gp_freq_background[i][j])
...
#endif
{
    ...
    do { LOCK(query_chunk_mutex);
        if (gv_chunk_nb >= gv_chunk_nb_end) {
            test = 1;UNLOCK(query_chunk_mutex);
        } else {
          ...
          fprintf(stderr,"[%ld/%ld]\n",current_chunk_nb+1,gp_nbchunks_query);
            */
            ...
            if (q_size < gp_seeds_span_min) {
                error("selected query chunk \"", gp_files[0],
                      "\" is too small to be indexed, please change the seed with the \'-p \"##-#\"\' parameter");
            }
...
int main(int argc, char *argv[]) {
    ...
    if (gp_nbfiles > 0) {
        if (gp_nbchunks_query == 1) {
            if (gp_chunksize_query[0] < gp_seeds_span_min) {
                error("first datafile \"", gp_files[0],
                      "\" is too small to be indexed with the current seed. Please change the seed with the \'-p \"##-#-##\"\' parameter");
            }
        } else {
            if (gp_nbchunks_query > 1) {
                int i;
                for (i = 0; i < gp_nbchunks_query; i++) {
                    if (gp_chunksize_query[i] < gp_seeds_span_min) {
                        error("first datafile \"", gp_files[0],
                              "\" contains too small chunks to be indexed with the current seed. Please change the seed with the \'-p \"##-#-##\"\' parameter");
                    }
                }
            } else {
    ...
    if (gp_nbfiles > 1) {
        if (gp_nbchunks_text == 1) {
            if (gp_chunksize_text[0] < gp_seeds_span_min) {
                error("second datafile \"", gp_files[1],
                      "\" is too small to be indexed with the current seed. Please change the seed with the \'-p \"##-#-##\"\' parameter");
            }
        } else {
            if (gp_nbchunks_text > 1) {
                ...
                for (i = 0; i < gp_nbchunks_text; i++) {
                    if (gp_chunksize_text[i] < gp_seeds_span_min) {
                        warning("second datafile \"", gp_files[1],
                                "\" contains small chunks that will be ignored using the current seed. Please change the seed with the \'-p \"##-#-##\"\' parameter\n");
                        break;
                    }
                }
            } else {
    ...
    if (gp_selection_fasta == 0) {
        ...
        /* if more than 4 chunks ... "-S 0" warning */
        if (gp_nbchunks_query > 4) {
            _WARNING(
                    "the -S 0 parameter enumerates all the first file chunks, indexing one at a time : it can be slow !!");
        }
    } else {
    ...
    gv_thread_num[0] = 0;
    thread_query_chunk((void *) (&(gv_thread_num[0])));     // main process
#endif
