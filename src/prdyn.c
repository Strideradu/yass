                            ...
                            Feature *feature)
{
  ...
  left_size  =  ma->left_pos_end  - TEL_POS(t_prev) + 1;
  right_size =  ma->right_pos_end - TER_POS(t_prev) + 1;
...
long int alignment_SG_stats_Border(char * data1, long int pos1, long int len1,
  ...
  long int k = 0;
  backtrack_SG *  bk = (backtrack_SG*) MALLOC( (len1+len2+1) * sizeof(backtrack_SG));
  ASSERT(bk,alignment_SG_stats_Border_noflush);
      ...
      if ( bk[k-1] == M_TO_M_SUBS || bk[k-1] == FIRST_DEL_LEFT || bk[k-1] == FIRST_DEL_RIGHT ) {
        ...
        if (
          ...
          if (
            ...
            _WARNING("alignment_SG_stats_Border_noflush() 1");
          }
        }
      }else{
        /* need to open a gap */
          ...
          if ( j + bandwidth < width - 1 &&
            ...
            if ( j + bandwidth > 0 &&
              ...
              if ( j + bandwidth < width - 1 &&
                ...
                _WARNING("alignment_SG_stats_Border_noflush() 2");
              }
            }
          }
        }
  ...
  /* [3] display alignment */
  {
    ...
    while (i1 < len1 || i2 < len2 ) {
      ...
      default:
        _WARNING("alignment_SG_stats_Border_noflush() 3");
      }/* case */
    }/* while */
    FREE(bk, (len1+len2+1) * sizeof(backtrack_SG));
  }
...
long int alignment_SG_score_on_MA(char * data1, char * data2,
  ...
  if ( left_size > 0 && right_size > 0)
    alignment_SG_score(data1, ma->left_pos_begin , left_size ,
                       ...
                       &(ma->blastscore));
#ifdef DEBUG_SCORE_MA
  fprintf(stderr,"score a gauche %ld\n",ma->blastscore);
#endif
  /* [2] Compute alignment for several tuples */
  ...
  while(t != NULL) {
    ...
    alignment_SG_score_Strait(data1,TBL_POS(t),
                              ...
                              &(ma->blastscore));
#ifdef DEBUG_SCORE_MA
    fprintf(stderr,"score apres strait: %ld length=%ld à(%ld-%ld)(%ld-%ld)\n",ma->blastscore,TSIZE(t),
            TBL_POS(t),TEL_POS(t),
            TBR_POS(t),TER_POS(t));
#endif
    if (t->next) {
      ...
      alignment_SG_score(data1,TEL_POS(t),TGAP_L(t,t->next),
                         ...
                         &(ma->blastscore));
#ifdef DEBUG_SCORE_MA
      fprintf(stderr,"score entre tuple: %ld\n",ma->blastscore);
#endif
    }/* if */
    t_prev = t;
    t = t->next;
  ...
  right_size =  ma->right_pos_end - TER_POS(t_prev);
#ifdef DEBUG_SCORE_MA
  fprintf(stderr,"score a avant à droite:%ld len1=%ld len2=%ld\n",ma->blastscore,left_size,right_size);
#endif
  ...
  if ( left_size > 0 && right_size > 0)
    alignment_SG_score(data1, TEL_POS(t_prev), left_size ,
                       ...
                       &(ma->blastscore));
#ifdef DEBUG_SCORE_MA
  printf(stderr,"score apres à droite:%ld\n",ma->blastscore);
#endif
  return 0;
}
...
long int alignment_SG_score_Strait(char * data1,  long int pos1, char * data2,  long int pos2, long int len,
  ...
  for (i=0;i<len;i++) {
    *p_score=*p_score-gp_substitution_matrix[((long int)(data1 + pos1)[i])][(long int)(data2 + pos2)[i]];
#ifdef DEBUG_SCORE_MA
